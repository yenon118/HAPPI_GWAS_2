...
def main(args):
	...
	#######################################################################
	# Generate and save BLUP configuration file
	#######################################################################
	blup_config_data = {
	...
	with open(output_folder.joinpath("Snakemake_BLUP_config.json"), 'w', encoding='utf-8') as writer:
		json.dump(blup_config_data, writer, ensure_ascii=False, indent=4)

	#######################################################################
	# Run BLUP in Snakemake
	#######################################################################

	# Construct snakemake command array
	snakemake_blup_command_array = [
		"snakemake",
		'--jobs', str(jobs),
		'--latency-wait', str(latency_wait),
		'--printshellcmds',
		'--snakefile', str(workflow_path.joinpath('rules', 'r_blup.smk')),
		'--configfile', str(output_folder.joinpath('Snakemake_BLUP_config.json'))
	]

	if keep_going is not None:
		if keep_going:
			snakemake_blup_command_array.append('--keep-going')

	if cluster is not None:
		if cluster != '':
			snakemake_blup_command_array.append('--executor cluster-generic --cluster-generic-submit-cmd')
			snakemake_blup_command_array.append("\""+str(cluster)+"\"")

	# Construct snakemake command string from snakemake command array
	snakemake_blup_command_string = str(' '.join(snakemake_blup_command_array))

	print("\n\n{}\n".format(snakemake_blup_command_string))

	# Split snakemake command string using shlex to prepare for subprocess
	snakemake_blup_command = shlex.split(snakemake_blup_command_string)

	try:
		# Run snakemake command using subprocess
		snakemake_blup_outcome = subprocess.run(
			snakemake_blup_command,
			capture_output=True,
			check=True,
	...
	try:
		with open(output_folder.joinpath("Snakemake_BLUP.log"), 'w') as writer:
			writer.write("\n\nStandard output: \n{} \n\nStandard error: \n{} \n\nCommand: \n{} \n\nReturn code: \n{} \n\n".format(snakemake_blup_outcome.stdout, snakemake_blup_outcome.stderr, snakemake_blup_outcome.args, snakemake_blup_outcome.returncode))
	except Exception as e:
		print(e)
		sys.exit(1)
...
if __name__ == "__main__":
	...
	# Parse arguments
	#######################################################################
	parser = argparse.ArgumentParser(prog='BLUP', description='BLUP')
