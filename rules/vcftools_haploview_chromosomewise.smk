...
rule vcftools_subset_and_convert_to_plink:
    ...
    shell:
        """
        if [[ {params.ulimit} -gt 0 ]]; then ulimit -Sn {params.ulimit}; fi;
        mkdir -p {params.out_folder};
        vcftools \
        --gzvcf {params.in_file} \
        --chr {params.chrom_start_end[0]} \
        --from-bp {params.chrom_start_end[1]} \
        --to-bp {params.chrom_start_end[2]} \
        --recode \
        --stdout | bgzip > {output.recode_vcf_file};
        vcftools \
        --gzvcf {output.recode_vcf_file} \
        --plink \
...
rule haploview:
    ...
    shell:
        ...
        -jar {workflow_path}/tools/Haploview.jar \
        -n \
        -out {params.output_prefix} \
        -pedfile {input.ped_file} \
        -map {input.map_file} \
        -skipcheck \
        -dprime \
        -png \
        ...
        -ldvalues DPRIME \
        -blockoutput GAB \
        -minMAF 0.05 | tee {log} {output.out_file};
        """
