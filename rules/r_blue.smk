...
rule all:
	input:
		expand(os.path.join(os.path.abspath(output_folder), '{sample}_BLUE.txt'), sample=samples)


## Run BLUE in a snakemake rule
rule r_blue:
	input:
	...
	shell:
		...
		Rscript {workflow_path}/scripts/R/BLUE.R -i {input.in_file} -o {params.out_folder} -e {params.feature_column_indexes} 2>&1 > {log}
		"""
