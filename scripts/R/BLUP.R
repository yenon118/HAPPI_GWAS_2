...
if (length(feature_column_indexes) > 0) {
	...
	for (i in 1:length(feature_column_indexes)) {
...
for(i in target_column_indexes){
	...
	# estimate BLUP
	blup_model <- lme4::ranef(lme)

	# extract BLUP and add grand mean when only one repetition present
	blup_model_out <- blup_model[[1]] + summary(lme)$coefficients[1]
	blup_model_out_dat <- as.data.frame(blup_model_out, check.names=FALSE, stringsAsFactors=FALSE)
	colnames(blup_model_out_dat) <- colnames(dat)[i]
	blup_model_out_dat <- tibble::rownames_to_column(blup_model_out_dat, var = "Accession")

	# Merge BLUP data
	if (is.null(blup_dat)) {
		blup_dat <- blup_model_out_dat
	} else {
		blup_dat <- dplyr::full_join(blup_dat, blup_model_out_dat)
	}
}

# Sort BLUP data
blup_dat <- arrange(blup_dat, Accession)
blup_dat <- as.data.frame(blup_dat, check.names=FALSE, stringsAsFactors=FALSE)
...
write.table(
	x = blup_dat,
	sep = "\t",
	file = file.path(output_path, paste0(file_prefix, "_BLUP.txt")),
	na = "",
	row.names = FALSE,
	quote = FALSE
)
